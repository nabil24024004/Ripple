"use strict";const{app:l,BrowserWindow:G,screen:T,ipcMain:u,shell:v,Tray:q,Menu:J,nativeImage:K,clipboard:C}=require("electron"),d=require("node:path"),f=require("fs"),E=require("os");process.platform==="linux"&&(l.commandLine.appendSwitch("enable-transparent-visuals"),l.commandLine.appendSwitch("disable-gpu-compositing"),l.disableHardwareAcceleration());let P=null,a=null;function H(){try{return d.join(l.getPath("userData"),"ripple-island.log")}catch{return d.join(process.cwd(),"ripple-island.log")}}function y(t,e=null){const s=`[${new Date().toISOString()}] ${t}${e?` | Error: ${e.stack||e}`:""}
`;console.log(s.trim());try{const i=H();f.appendFileSync(i,s,"utf8")}catch{}}process.on("uncaughtException",t=>{y("UNCAUGHT EXCEPTION (Main Process)",t)});process.on("unhandledRejection",t=>{y("UNHANDLED REJECTION (Main Process)",t)});const{exec:p,execFile:g,spawn:A}=require("child_process");function k(t){return Buffer.from(t,"utf16le").toString("base64")}function V(){return new Promise(t=>{const n=Buffer.from(`
...
Icon=${D()}
Terminal=false
`;f.writeFileSync(s,i)}else f.existsSync(s)&&f.unlinkSync(s)}catch(i){console.error("Failed to set auto-launch on Linux:",i)}}else if(process.platform==="win32")try{l.setLoginItemSettings({openAtLogin:e,path:l.getPath("exe")})}catch(n){console.error("Failed to set login item settings on Windows:",n)}});const D=()=>{const t="png";if(l.isPackaged){const e=d.join(process.resourcesPath,`icon.${t}`),n=d.join(process.resourcesPath,`assets/icons/icon.${t}`);return f.existsSync(e)?e:f.existsSync(n)?n:e}return d.join(__dirname,`../../src/assets/icons/icon.${t}`)},_=()=>{const t=T.getPrimaryDisplay(),{x:e,y:n,width:s,height:i}=t.bounds,o=process.platform==="linux",r=process.platform==="win32",c=process.platform==="darwin",m=s,$=i,h=e,S=n,L=r?"toolbar":"panel";a=new G({width:m,height:$,x:h,y:S,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...r?{thickFrame:!1}:{},hasShadow:!1,skipTaskbar:!0,icon:D(),...c?{hiddenInMissionControl:!0}:{},type:L,fullscreen:!1,visibleOnFullScreen:!0,acceptFirstMouse:!0,webPreferences:{preload:d.join(__dirname,"preload.js"),devTools:!l.isPackaged},show:!0}),o?a.setIgnoreMouseEvents(!0):a.setIgnoreMouseEvents(!0,{forward:!0});const B=o?500:0;a.once("ready-to-show",()=>{setTimeout(()=>{a&&(a.show(),o?a.setAlwaysOnTop(!0,"screen-saver"):a.setAlwaysOnTop(!0,"pop-up-menu"),a.focus())},B)}),setTimeout(()=>{a&&!a.isVisible()&&(a.show(),a.focus())},5e3),a.on("closed",()=>{a=null});try{a.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!l.isPackaged)a.loadURL("http://localhost:5173");else if(!l.isPackaged&&process.env.NODE_ENV==="development")a.loadURL("http://localhost:5173");else{const R=[d.join(__dirname,"../renderer/main_window/index.html"),d.join(l.getAppPath(),".vite/renderer/main_window/index.html"),d.join(l.getAppPath(),"dist/index.html"),d.join(__dirname,"index.html"),d.join(l.getAppPath(),"index.html")];let F=!1;for(const N of R)if(f.existsSync(N)){y(`Loading renderer from: ${N}`),a.loadFile(N),F=!0;break}F||(y("No packaged renderer HTML found, falling back to localhost:5173"),a.loadURL("http://localhost:5173"))}};l.whenReady().then(()=>{process.platform==="darwin"&&l.dock.hide(),_(),ne(),se(),l.on("activate",()=>{G.getAllWindows().length===0&&_()});try{const t=D(),n=K.createFromPath(t).resize({width:16,height:16});P=new q(n);const s=J.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{a&&(a.isVisible()?a.hide():a.show())}},{type:"separator"},{label:"Quit",click:()=>{l.quit()}}]);P.setToolTip("Ripple"),P.setContextMenu(s)}catch(t){console.error("Failed to create tray:",t)}});const U=d.join(l.getPath("userData"),"get-media.ps1"),X=`[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
...
`,i=k(s);p(`powershell -NoProfile -EncodedCommand ${i}`,(o,r)=>{n((r==null?void 0:r.trim())==="true")})}));u.handle("focus-notification-app",async(t,e)=>process.platform!=="win32"||!e?!1:new Promise(n=>{const s=e.split("!")[0].split("_")[0].replace(/\./g,""),i=`
...
`,o=k(i);p(`powershell -NoProfile -EncodedCommand ${o}`,(r,c)=>{n((c==null?void 0:c.trim())==="true")})}));
