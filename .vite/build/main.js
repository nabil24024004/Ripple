"use strict";const{app:l,BrowserWindow:G,screen:P,ipcMain:d,shell:O,Tray:q,Menu:J,nativeImage:K,clipboard:k}=require("electron"),u=require("node:path"),f=require("fs"),v=require("os");process.platform==="linux"&&(l.commandLine.appendSwitch("enable-transparent-visuals"),l.commandLine.appendSwitch("disable-gpu-compositing"),l.disableHardwareAcceleration());let T=null,a=null;function H(){try{return u.join(l.getPath("userData"),"ripple-island.log")}catch{return u.join(process.cwd(),"ripple-island.log")}}function y(t,e=null){const s=`[${new Date().toISOString()}] ${t}${e?` | Error: ${e.stack||e}`:""}
`;console.log(s.trim());try{const i=H();f.appendFileSync(i,s,"utf8")}catch{}}process.on("uncaughtException",t=>{y("UNCAUGHT EXCEPTION (Main Process)",t)});process.on("unhandledRejection",t=>{y("UNHANDLED REJECTION (Main Process)",t)});const{exec:p,execFile:w,spawn:A}=require("child_process");function E(t){return Buffer.from(t,"utf16le").toString("base64")}function V(){return new Promise(t=>{const n=Buffer.from(`
...
`,"utf16le").toString("base64");p(`powershell -NoProfile -EncodedCommand ${n}`,{maxBuffer:5*1024*1024},(s,i)=>{if(s||!i)return t([]);try{const o=JSON.parse(i.trim());t(Array.isArray(o)?o:o?[o]:[])}catch{t([])}})})}function Z(){return new Promise(t=>{const n=Buffer.from(`
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$results = [System.Collections.Generic.List[object]]::new()
...
@($results) | ConvertTo-Json -Compress -Depth 2
`,"utf16le").toString("base64");p(`powershell -NoProfile -EncodedCommand ${n}`,{maxBuffer:2*1024*1024},(s,i)=>{if(s||!i)return t([]);try{const o=JSON.parse(i.trim());t(Array.isArray(o)?o:o?[o]:[])}catch{t([])}})})}async function z(){const[t,e]=await Promise.all([V(),Z()]),n=new Set,s=[];for(const i of[...t,...e]){if(!i.name||!(i.path||i.appId))continue;const o=i.type==="uwp"?`shell:AppsFolder\\${i.appId}`:i.path,r=o.toLowerCase();n.has(r)||(n.add(r),s.push({name:i.name,launch:o}))}return s.sort((i,o)=>i.name.localeCompare(o.name))}function N(t){const e=[];let n=0;for(;n<t.length;){for(;n<t.length&&/\s/.test(t[n]);)n++;if(n>=t.length)break;let s="";for(;n<t.length&&!/\s/.test(t[n]);)if(t[n]==='"'){for(n++;n<t.length&&t[n]!=='"';)s+=t[n++];n<t.length&&n++}else s+=t[n++];s&&e.push(s)}return e}function Q(t){const e=t.replace(/\//g,"\\").replace(/%([^%]+)%/g,(o,r)=>process.env[r]||`%${r}%`),n=e.match(/^"([^"]+)"(.*)/);if(n)return{exe:n[1],args:n[2].trim()?N(n[2].trim()):[]};const s=e.match(/^(.+?\.(?:exe|cmd|bat|com|ps1))(?:\s+(.*))?$/i);if(s)return{exe:s[1],args:s[2]?N(s[2]):[]};const i=e.search(/\s/);return i===-1?{exe:e,args:[]}:{exe:e.slice(0,i),args:N(e.slice(i+1).trim())}}function Y(t){const e=t.trim();if(e.startsWith("shell:")){const n=e.replace(/'/g,"''");p(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${n}'"`);return}if(/[\\\/]/.test(e)){const{exe:n,args:s}=Q(e);if(s.length===0){if(n.toLowerCase().endsWith(".url")){try{const c=f.readFileSync(n,"utf8").match(/^URL=(.+)$/im);c&&O.openExternal(c[1].trim())}catch{}return}O.openPath(n).then(r=>{r&&p(`start "" "${n}"`)});return}const i=/[\\/]/.test(n)&&!/\.[^\\.]+$/.test(n)?n+".exe":n;if(/\.(cmd|bat)$/i.test(i)){const r=A("cmd.exe",["/c",i,...s],{shell:!1,detached:!0,stdio:"ignore"});r.on("error",()=>{}),r.unref();return}if(/\.ps1$/i.test(i)){const r=A("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-File",i,...s],{shell:!1,detached:!0,stdio:"ignore"});r.on("error",()=>{}),r.unref();return}const o=A(i,s,{shell:!1,detached:!0,stdio:"ignore"});o.on("error",()=>{}),o.unref();return}if(e.includes(" ")){const n=e.replace(/'/g,"''");p(`powershell -NoProfile -WindowStyle Hidden -Command "Start-Process '${n}'"`)}else p(`start "" ${e}`)}d.handle("log-message",(t,e,n,s)=>{y(`[RENDERER ${String(e).toUpperCase()}] ${n} ${s?JSON.stringify(s):""}`)});d.handle("set-ignore-mouse-events",(t,e,n)=>{if(a&&!a.isDestroyed())try{if(process.platform!=="linux"){const s=n!==void 0?n:e;a.setIgnoreMouseEvents(e,{forward:s}),y(`setIgnoreMouseEvents(${e}, forward=${s}) executed`)}else a.setIgnoreMouseEvents(e),y(`setIgnoreMouseEvents(${e}) executed (linux)`)}catch(s){y("Error in setIgnoreMouseEvents, falling back to forward=true",s);try{a.setIgnoreMouseEvents(!0,{forward:!0})}catch{}}});d.handle("focus-window",()=>{a&&a.focus()});d.handle("open-external",async(t,e)=>{await O.openExternal(e)});d.handle("launch-app",async(t,e)=>{const n=process.platform;n==="darwin"?p(`open -a "${e}"`):n==="win32"?Y(e):p(e)});d.handle("build-app-cache",async()=>{if(process.platform!=="win32")return;const t=u.join(l.getPath("userData"),"app-cache.json");try{const e=await z();f.writeFileSync(t,JSON.stringify(e))}catch{}});d.handle("search-apps",async(t,e)=>{if(process.platform!=="win32"||!e)return[];const n=u.join(l.getPath("userData"),"app-cache.json");try{if(!f.existsSync(n))return[];const s=JSON.parse(f.readFileSync(n,"utf8")),i=e.toLowerCase();return s.filter(o=>o.name&&o.name.toLowerCase().includes(i)).slice(0,8)}catch{return[]}});d.handle("get-displays",()=>P.getAllDisplays().map(e=>({id:e.id,label:e.label||`Display ${e.id}`,bounds:e.bounds})));d.handle("set-display",(t,e)=>{if(a){const s=P.getAllDisplays().find(m=>m.id.toString()===e.toString())||P.getPrimaryDisplay(),{x:i,y:o,width:r,height:c}=s.bounds;process.platform,a.setBounds({x:i,y:o,width:r,height:c}),a.show()}});d.handle("update-window-position",(t,e,n)=>{});d.handle("set-auto-launch",(t,e)=>{if(process.platform==="linux"){const n=u.join(l.getPath("home"),".config","autostart"),s=u.join(n,"ripple.desktop");try{if(e){f.existsSync(n)||f.mkdirSync(n,{recursive:!0});const i=`[Desktop Entry]
Type=Application
Version=1.0
Name=Ripple
...
Icon=${R()}
Terminal=false
`;f.writeFileSync(s,i)}else f.existsSync(s)&&f.unlinkSync(s)}catch(i){console.error("Failed to set auto-launch on Linux:",i)}}else if(process.platform==="win32")try{l.setLoginItemSettings({openAtLogin:e,path:l.getPath("exe")})}catch(n){console.error("Failed to set login item settings on Windows:",n)}});const R=()=>{const t="png";if(l.isPackaged){const e=u.join(process.resourcesPath,`icon.${t}`),n=u.join(process.resourcesPath,`assets/icons/icon.${t}`);return f.existsSync(e)?e:f.existsSync(n)?n:e}return u.join(__dirname,`../../src/assets/icons/icon.${t}`)},_=()=>{const t=P.getPrimaryDisplay(),{x:e,y:n,width:s,height:i}=t.bounds,o=process.platform==="linux",r=process.platform==="win32",c=process.platform==="darwin",m=s,$=i,h=e,S=n,j=r?"toolbar":"panel";a=new G({width:m,height:$,x:h,y:S,backgroundColor:"#00000000",transparent:!0,alwaysOnTop:!0,resizable:!1,frame:!1,...r?{thickFrame:!1}:{},hasShadow:!1,skipTaskbar:!0,icon:R(),...c?{hiddenInMissionControl:!0}:{},type:j,fullscreen:!1,visibleOnFullScreen:!0,acceptFirstMouse:!0,webPreferences:{preload:u.join(__dirname,"preload.js"),devTools:!l.isPackaged},show:!0}),o?a.setIgnoreMouseEvents(!0):a.setIgnoreMouseEvents(!0,{forward:!0});const B=o?500:0;a.once("ready-to-show",()=>{setTimeout(()=>{a&&(a.show(),o?a.setAlwaysOnTop(!0,"screen-saver"):a.setAlwaysOnTop(!0,"pop-up-menu"),a.focus())},B)}),setTimeout(()=>{a&&!a.isVisible()&&(a.show(),a.focus())},5e3),a.on("closed",()=>{a=null});try{a.setVisibleOnAllWorkspaces(!0,{visibleOnFullScreen:!0})}catch{}if(!l.isPackaged)a.loadURL("http://localhost:5173");else if(!l.isPackaged&&process.env.NODE_ENV==="development")a.loadURL("http://localhost:5173");else{const F=[u.join(__dirname,"../renderer/main_window/index.html"),u.join(l.getAppPath(),".vite/renderer/main_window/index.html"),u.join(l.getAppPath(),"dist/index.html"),u.join(__dirname,"index.html"),u.join(l.getAppPath(),"index.html")];let D=!1;for(const C of F)if(f.existsSync(C)){y(`Loading renderer from: ${C}`),a.loadFile(C),D=!0;break}D||(y("No packaged renderer HTML found, falling back to localhost:5173"),a.loadURL("http://localhost:5173"))}};l.whenReady().then(()=>{process.platform==="darwin"&&l.dock.hide(),_(),ne(),se(),l.on("activate",()=>{G.getAllWindows().length===0&&_()});try{const t=R(),n=K.createFromPath(t).resize({width:16,height:16});T=new q(n);const s=J.buildFromTemplate([{label:"Show/Hide Ripple",click:()=>{a&&(a.isVisible()?a.hide():a.show())}},{type:"separator"},{label:"Quit",click:()=>{l.quit()}}]);T.setToolTip("Ripple"),T.setContextMenu(s)}catch(t){console.error("Failed to create tray:",t)}});const U=u.join(l.getPath("userData"),"get-media.ps1"),X=`[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
...
Write-Output "null"
`;try{f.writeFileSync(U,X,"utf8")}catch(t){console.error("Failed to write get-media.ps1 script:",t)}let W=!1;d.handle("get-system-media",async()=>W?null:(W=!0,new Promise(t=>{const e=s=>{W=!1,t(s)},n=process.platform;n==="darwin"?w("osascript",["-e",`
...
`,"utf16le").toString("base64");p(`powershell -NoProfile -EncodedCommand ${s}`,(i,o)=>{if(i||!o)return t({connected:!1,devices:[]});try{const r=JSON.parse(o.trim());t({connected:!!r.connected,devices:Array.isArray(r.devices)?r.devices:r.devices?[r.devices]:[]})}catch{t({connected:!1,devices:[]})}})}else e==="linux"?p("bluetoothctl devices Connected",(n,s)=>{if(n)return t(!1);t(s.trim().length>0)}):t(!1)}));d.handle("get-camera-status",async()=>new Promise(t=>{const e=process.platform;e==="darwin"?p('ioreg -l | grep -E "FrontCameraActive|FrontCameraStreaming"',(n,s)=>{t(s?s.includes("= Yes"):!1)}):e==="win32"?w("reg",["query","HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\webcam","/s"],(n,s)=>{if(n||!s)return t(!1);const i=s.match(/LastUsedTimeStop\s+REG_QWORD\s+0x0\b/i);t(!!i)}):e==="linux"?p("fuser /dev/video* 2>/dev/null",(n,s)=>{t(s.trim().length>0)}):t(!1)}));d.handle("get-microphone-status",async()=>new Promise(t=>{const e=process.platform;e==="darwin"?p('ioreg -l | grep -E "IOAudioStreamActive|IOAudioEngine|IOAudioStream" | grep -i "Yes"',(n,s)=>{t(s?s.trim().length>0:!1)}):e==="win32"?w("reg",["query","HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone","/s"],(n,s)=>{if(n||!s)return t(!1);const i=s.match(/LastUsedTimeStop\s+REG_QWORD\s+0x0\b/i);t(!!i)}):e==="linux"?p("pactl list source-outputs | grep -q 'Source #'",n=>{t(!n)}):t(!1)}));l.on("window-all-closed",()=>{process.platform==="linux"&&!T&&l.quit()});d.handle("control-system-media",async(t,e)=>{const n=process.platform;if(n==="darwin"){const s=`
        ...
        `;w("osascript",["-e",s])}else if(n==="win32"){let s="0xCD";e==="next"&&(s="0xB0"),e==="previous"&&(s="0xB1"),y(`Executing media control: ${e} (${s})`);const i=`
...
`,o=E(i);p(`powershell -NoProfile -ExecutionPolicy Bypass -EncodedCommand ${o}`,r=>{r&&y("Media control error:",r)})}else if(n==="linux"){let s=e;e==="playpause"&&(s="play-pause"),p(`playerctl ${s}`)}});let g=null;function ee(){const t=v.cpus();if(!t||t.length===0)return 0;if(!g||g.length!==t.length)return g=t,0;let e=0,n=0;for(let i=0;i<t.length;i++){const o=t[i],r=g[i];let c=o.times.idle-r.times.idle,m=0;for(const $ in o.times)m+=o.times[$]-r.times[$];e+=c,n+=m}if(g=t,n===0)return 0;const s=e/n;return Math.max(0,Math.min(100,Math.round((1-s)*100)))}function te(){const t=v.totalmem(),e=v.freemem();return t?Math.max(0,Math.min(100,Math.round((t-e)/t*100))):0}d.handle("get-system-metrics",async()=>({cpu:ee(),ram:te()}));d.handle("get-clipboard-text",async()=>{try{return k.readText()}catch{return""}});d.handle("write-clipboard-text",async(t,e)=>{try{return e?k.writeText(e):k.clear(),!0}catch{return!1}});d.handle("clear-clipboard",async()=>{try{return k.clear(),!0}catch{return!1}});d.handle("control-system-volume",async(t,e)=>{if(process.platform==="win32"){let n="0xAF";e==="down"&&(n="0xAE"),e==="mute"&&(n="0xAD");const s=`[Void][System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); $type = '[DllImport("user32.dll")] public static extern void keybd_event(byte bVk, byte bScan, uint dwFlags, UIntPtr dwExtraInfo);'; $MediaKey = Add-Type -MemberDefinition $type -Name "WinMediaKey" -Namespace "WinAPI" -PassThru; $MediaKey::keybd_event(${n}, 0, 0, [UIntPtr]::Zero); $MediaKey::keybd_event(${n}, 0, 2, [UIntPtr]::Zero);`;return p(`powershell -NoProfile -Command "${s}"`),!0}return!1});let x=null,I=null;function ne(){if(process.platform!=="win32")return;let t=!1;setInterval(()=>{!a||a.isDestroyed()||t||(t=!0,p('powershell -NoProfile -Command "[Console]::CapsLock; [Console]::NumberLock"',(e,n)=>{var r,c;if(t=!1,e||!n)return;const s=n.trim().split(/\r?\n/),i=((r=s[0])==null?void 0:r.trim().toLowerCase())==="true",o=((c=s[1])==null?void 0:c.trim().toLowerCase())==="true";x!==null&&i!==x&&a&&!a.isDestroyed()&&a.webContents.send("key-lock-change",{key:"CapsLock",state:i}),I!==null&&o!==I&&a&&!a.isDestroyed()&&a.webContents.send("key-lock-change",{key:"NumLock",state:o}),x=i,I=o}))},2e3)}let b=null;function se(){if(process.platform!=="win32")return;let t=!1;setInterval(()=>{!a||a.isDestroyed()||t||(t=!0,p("wmic logicaldisk where drivetype=2 get name,volumename /format:csv",(e,n)=>{var o,r;if(t=!1,e)return;const s=new Map,i=n.trim().split(/\r?\n/).filter(c=>c.trim()&&!c.startsWith("Node"));for(const c of i){const m=c.trim().split(",");if(m.length>=3){const $=(o=m[1])==null?void 0:o.trim(),h=((r=m[2])==null?void 0:r.trim())||"USB Drive";$&&s.set($,h)}}if(b!==null&&a&&!a.isDestroyed()){for(const[c,m]of s)b.has(c)||a.webContents.send("usb-change",{action:"connected",drive:c,name:m});for(const[c]of b)s.has(c)||a.webContents.send("usb-change",{action:"disconnected",drive:c,name:"USB Drive"})}b=s}))},5e3)}const L=u.join(l.getPath("userData"),"get-notifications.ps1"),ie=`[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
Add-Type -AssemblyName System.Runtime.WindowsRuntime
...
`;try{f.writeFileSync(L,ie,"utf8")}catch(t){y("Failed to write get-notifications.ps1:",t)}let M=!1;d.handle("get-notifications",async()=>process.platform!=="win32"?[]:M?[]:(M=!0,new Promise(t=>{w("powershell",["-NoProfile","-ExecutionPolicy","Bypass","-File",L],{maxBuffer:5*1024*1024,encoding:"utf8",timeout:1e4},(e,n)=>{if(M=!1,e||!n)return t([]);try{const s=JSON.parse(n.trim());t(Array.isArray(s)?s:s?[s]:[])}catch{t([])}})})));d.handle("dismiss-notification",async(t,e)=>process.platform!=="win32"?!1:new Promise(n=>{const s=`
Add-Type -AssemblyName System.Runtime.WindowsRuntime
$asTaskGeneric = [System.WindowsRuntimeSystemExtensions].GetMethods() | Where-Object {
    $_.Name -eq 'AsTask' -and $_.IsGenericMethodDefinition -and $_.GetGenericArguments().Count -eq 1 -and $_.GetParameters().Count -eq 1 -and $_.GetParameters()[0].ParameterType.Name.StartsWith('IAsyncOperation')
}[0]
...
`,i=E(s);p(`powershell -NoProfile -EncodedCommand ${i}`,(o,r)=>{n((r==null?void 0:r.trim())==="true")})}));d.handle("focus-notification-app",async(t,e)=>process.platform!=="win32"||!e?!1:new Promise(n=>{const s=e.split("!")[0].split("_")[0].replace(/\./g,""),i=`
...
`,o=E(i);p(`powershell -NoProfile -EncodedCommand ${o}`,(r,c)=>{n((c==null?void 0:c.trim())==="true")})}));
